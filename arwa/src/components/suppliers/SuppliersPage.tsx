...
interface Analytics {
  ...
  totalRemainingToSuppliers: number;
  netProfit: number;
}
...
export const SuppliersPage: React.FC<SuppliersPageProps> = ({
  ...
  const [suppliersData, setSuppliersData] =
    useState<Supplier[]>(initialSuppliersData);
  ...
  const getStockStatus = (supplied: number, sold: number): StockStatus => {
  ...
  const handleEditPayment = (supplier: Supplier) => {
  ...
  const handleEditPrice = (supplier: Supplier) => {
