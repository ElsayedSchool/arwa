...
import type { Customer, CreateCustomerData, UpdateCustomerData } from "./models/customer";
import CustomerModal from "./components/CustomerModal";
import CustomersAnalysis from "./components/CustomersAnalysis";
import CustomersTable from "./components/CustomersTable";

const CustomersPage: React.FC = () => {
  const { customers, loading, error, searchTerm, deleteCustomer, setSearchTerm, clearError } =
    useCustomerStore();
  ...
  const handleSaveCustomer = async (data: CreateCustomerData | UpdateCustomerData) => {
    if (editingCustomer) {
      await useCustomerStore.getState().updateCustomer(data as UpdateCustomerData);
    } else {
      await useCustomerStore.getState().createCustomer(data as CreateCustomerData);
    }
  };
    ...
    <div className="min-h-screen bg-gray-50" dir="rtl">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="bg-white rounded-lg shadow-sm p-6 overflow-x-auto">
          <h3 className="text-lg font-semibold text-gray-900 mb-6">قائمة العملاء</h3>
          <CustomersTable
