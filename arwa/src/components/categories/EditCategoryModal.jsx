...
export const EditCategoryModal = ({ isOpen, onClose, onSave, editingItem }) => {
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={handleClose}
      title={editingItem.type === 'main' ? 'تعديل الفئة الرئيسية' : 'تعديل الفئة الفرعية'}
    >
      <div className="space-y-4">
        <Input
          label={editingItem.type === 'main' ? 'اسم الفئة الرئيسية' : 'اسم الفئة الفرعية'}
          value={formData.name}
          onChange={(e) => handleInputChange('name', e.target.value)}
          placeholder={editingItem.type === 'main' ? 'مثال: أسماك المياه العذبة' : 'مثال: بلطي'}
          error={errors.name}
        />
        ...
        {editingItem.type === 'sub' && (
          <>
            <Input
              ...
              onChange={(e) => handleInputChange('character', e.target.value.slice(0, 1))}
