...
interface AddInventoryModalProps {
  ...
  onClose: () => void;
  onSave?: (
    data: Omit<InventoryDeliveryUi, "id" | "lastEditTime">
  ) => Promise<void>;
  onSaveEdit?: (data: InventoryDeliveryUi) => void;
  suppliers: string[];
  types: CategoryDto[];
  isEdit?: boolean;
  existingDelivery?: InventoryDeliveryUi | null;
}
  ...
  const handleSubmit = () => {
    if (validateForm()) {
      ...
      if (isEdit && onSaveEdit && existingDelivery) {
        ...
        const editData: InventoryDeliveryUi = {
          ...existingDelivery,
          ...deliveryData,
        ...
        // For add, call onSave
        onSave(
          deliveryData as unknown as Omit<
            InventoryDeliveryUi,
            "id" | "lastEditTime"
          >
        );
