...
interface OrdersToConfirmProps {
  orders: Order[];
  onApprove: (orderId: number) => void;
  onReject: (orderId: number) => void;
  onEdit: (orderId: number, updatedData: Partial<Order>) => void;
}

export const OrdersToConfirm: React.FC<OrdersToConfirmProps> = ({
  orders,
  onApprove,
  onReject,
  onEdit,
  ...
  const handleEditSave = (orderId: number, updatedData: Partial<Order>) => {
  ...
  if (orders.length === 0) {
    return (
      <div className="bg-white rounded-lg shadow-sm p-8 text-center">
        ...
        <p className="text-gray-600">لا توجد طلبات معلقة لهذا العميل</p>
      </div>
    );
  }
    ...
    <div className="bg-white rounded-lg shadow-sm">
      ...
      {editingOrder && (
        <EditOrderModal
          order={editingOrder}
          setOrder={
            setEditingOrder as React.Dispatch<React.SetStateAction<Order>>
          }
          userRole="admin"
