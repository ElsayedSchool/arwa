...
import { UpsertEmployeeCommand } from "./upsertEmployee.Command";
...
@Injectable()
export class UpsertEmployeeHandler {
  constructor(
  ...
  async handle(command: UpsertEmployeeCommand, photos: Express.Multer.File[]) {
    let userData: User;
    const oldPhoto = "";
    if (command.id) {
  ...
  async onCreateNewUser(command: UpsertEmployeeCommand) {
  ...
  async onUpdateUser(command: UpsertEmployeeCommand) {
